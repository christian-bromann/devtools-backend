...
export function getInlineStylesForNode ({ nodeId }) {
    ...
    return { inlineStyle: cssStyle.rules[0].style }
}
...
export function styleSheetRegistered ({ header, cssText, url, ownerNode }) {
    /**
     * wait until document ready so `document.styleSheets` is not empty
     */
    if (!this.readyStateComplete) {
        return setTimeout(() => styleSheetRegistered.call(this, { header, cssText, url }), 100)
    }

    const cssStyleSheets = [].slice.call(document.styleSheets)
    const styleSheet = arrayFind(
        cssStyleSheets,
