...
window.CSSRuleList.prototype.toArray = function () {
    const returnValue = []
...
export function getMatchedStylesForNode ({ nodeId }) {
    ...
    const matchedCSSRules = ruleList.map((rule) => {
        ...
        const cssText = parseCss(rule.cssText)
        let styleSheetId = this.requestId.split('.')
        styleSheetId = styleSheetId[0] + '.' + (parseInt(styleSheetId[1]) + 10)

        const shorthandEntries = cssText.stylesheet.rules[0].declarations
            /**
...
export function getStyleSheetText ({ styleSheetId }) {
    ...
    const stylesheet = stylesheets.filter((sheet) => sheet.ownerNode._styleSheetId === styleSheetId)[0]

    console.log('issa da', !!stylesheet)
    ...
    if (stylesheet.ownerNode.nodeName.toLowerCase() === 'style') {
        return { text: stylesheet.ownerNode.textContent }
    }

    /**
     * generate stylesheet text based of css rules
     */
    const cssRules = [].slice.call(stylesheet.cssRules).map(
