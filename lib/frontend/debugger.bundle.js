...
document.onreadystatechange = function () {
    if (document.readyState === 'complete') {
        ...
        /**
         * register app manager for non HbbTV applications
         */
        const appMan = document.getElementById('debuggerAppMan')
        if (appMan) {
            if (typeof appMan.getOwnerApplication !== 'function') {
                return remoteDebugger.emit(
                    'debug',
                    'Application manager couldn\'t be initiated, TV doesn\'t support "getOwnerApplication"'
                )
            }
