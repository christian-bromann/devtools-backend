...
const program = async function (devtoolsBackendHost, uuid, currentScript, nextSibling) {
    /**
     * register TV
     */
...
function run () {
    ...
    program(devtoolsBackendHost, uuid, currentScript, nextSibling).catch((error) => {
        console.error('Something went wrong connecting to devtools backend:', error.message)
    })
}

if (false && document.currentScript) {
    run()
} else {
