...
export default class DevtoolsBackend {
    ...
    register (req, res) {
        ...
        const { uuid, hostname, url, title, description, metadata } = req.body
        ...
        const page = new Page(this.io, uuid, hostname, url, title, description, metadata)
        this.backend = new DebuggerService(page)
        this.backend.frameStartedLoading({'set-cookie': ['frameId=1']}) // emulate page load
        this.pages.push(page)

        /**
         * remove page if disconnected from devtools frontend
         */
        page.on('disconnect', (uuid) => {
