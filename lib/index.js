...
const PAGES_TPL_PATH = path.resolve(__dirname, '..', 'views', 'pages.tpl.html')
...
export default class DevtoolsBackend {
    constructor (host = DEFAULT_HOST, port = DEFAULT_PORT) {
        ...
        this.app.get('/settings', this.filterRequests(::this.getSettings))
        this.app.post('/settings', this.filterRequests(::this.postSettings))
    ...
    json (req, res) {
        ...
        return res.send(JSON.stringify(this.backend.pages.map((page) => {
            ...
            const title = page.title || page.url.host.split('.').slice(-2).join('.')
            return {
    ...
    getSettings (req, res) {
        return res.render('settings.tpl.html', readConfig())
    }
