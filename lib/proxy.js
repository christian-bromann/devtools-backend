...
export default class Proxy {
    constructor (app, backend) {
        ...
        this.page = this.backend.addPage({
            uuid: this.uuid,
            hostname: `${process.env.PROXY_NETWORK_ADDRESS}:${DEFAULT_PORT}`
        })
    ...
    proxy (req, proxyRes) {
        ...
        this.page.loaded(req.target, req.cookies.frameId)
    ...
    proxyFilter (req, proxyRes, next) {
        ...
        request.head(req.target, (err, res, body) => {
            ...
            if (contentType && contentType.match(/hbbtv/)) {
                return next()
            }
    ...
    async preflightCheck () {
        ...
        if (!await isOnline()) {
            this.log.error(
                'Couldn\'t connect to the internet. Proxy requires an internet connection to work.'
            )
        }
    }
}
