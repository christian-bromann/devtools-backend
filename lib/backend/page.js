...
export default class Page extends EventEmitter {
    ...
    connectWebSocket (ws) {
        ...
        this.ws = ws
        this.ws.on('message', this.wsHandler)
    ...
    handleIncomming (page, payload) {
        ...
        if (method === 'enable' && this.isDomainSupported(domain)) {
            this.enable(domain)
            return page.send({ id: msg.id, params: {} })
        }
        ...
        if (method === 'disable') {
            this.disable(domain)
            return page.emit({ id: msg.id, params: {} })
        }
